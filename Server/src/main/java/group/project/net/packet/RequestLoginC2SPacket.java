...
public class RequestLoginC2SPacket extends Packet {
    ...
    public void handle(Connection connection) throws IOException {
        if (this.principal.equals("admin@gmail.com") && this.password.equals("admin")) {
            ...
            try (FileReader reader = new FileReader("Server/src/main/admin/transactions.json")){
                Wallet wallet = new Wallet();
                wallet.read(JsonParser.parseReader(reader).getAsJsonObject());
                connection.send(new EvaluateWalletS2CPacket(wallet));
            } catch (Exception e) {
                e.printStackTrace();
            }

            try (FileReader reader = new FileReader("Server/src/main/admin/grades.json")) {
                Semester[] semesters = {};
                Program program = new Program(semesters);
                program.read(JsonParser.parseReader(reader).getAsJsonObject());
